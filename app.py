...
def init_session_state():
    ...
    # Inicjalizacja domyślnych wartości (tylko brakujących)
    from renderer import defaults
    for k, v in defaults.items():
        if k not in st.session_state: 
            st.session_state[k] = v

# ==========================================
# 2. FUNKCJE POMOCNICZE (LOGIKA BIZNESOWA UI)
# ==========================================
...
def get_project_filename():
    ...
    if m_date:
        mm, yy = m_date.group(1).zfill(2), m_date.group(2)[-2:]
...
def load_project_data(data: dict):
    img_keys = get_image_keys()
    for k, v in data.items():
        if k in img_keys and isinstance(v, str):
...
def section_template_manager(section_keys, file_prefix, default_filename, uploader_key, index=None):
    ...
    with c2:
        ...
        from renderer import COUNTRIES_DICT
        cc = st.session_state.get('country_code', 'OTH')
...
# ==========================================
# 3. KOMPONENTY WIDOKU PANELU BOCZNEGO
# ==========================================
...
def main():
    ...
    with st.sidebar:
        ...
        elif page == "Jak lecimy?": render_tab_flight()
        elif page == "Zakwaterowanie": render_tab_hotels()
        elif page == "Program Wyjazdu": render_tab_program()
        elif page == "Opis miejsc": render_tab_places()
        elif page == "Opis atrakcji": render_tab_attractions()
        elif page == "Aplikacja (Komunikacja)": render_tab_app()
        elif page == "Materiały Brandingowe": render_tab_branding()
        elif page == "Wirtualny Asystent": render_tab_va()
        elif page == "Pillow Gifts": render_tab_pillow()
        elif page == "Kosztorys": render_tab_costs()
        elif page == "Co o nas mówią": render_tab_testimonials()
        elif page == "O Nas (Zespół)": render_tab_team()
        elif page == "Wygląd i Kolory": render_tab_styling()
        elif page == "Zapisz / Wczytaj Projekt": render_tab_project_mgmt()
