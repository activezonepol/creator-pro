...
# ---------------------------------------------------------------------------
# SIDEBAR — NAWIGACJA Z MIEJSCAMI I ATRAKCJAMI
# ---------------------------------------------------------------------------
with st.sidebar:
    _acc = st.session_state.get('color_accent', '#FF6600')
    ...
    page = st.radio("WYBIERZ SEKCJE DO EDYCJI:", _nav_all, index=_default_idx, format_func=nav_format)
    ...
    if _pa_nav_items:
        ...
        for _spos, _sit in enumerate(_pa_nav_items):
            ...
            _sclr = _tc[_styp]
            _page_key = _pa_page_name(_styp, _sidx)
            _is_active = (_last == _page_key)
            
            _sc1, _sc2, _sc3, _sc4 = st.columns([6, 1, 1, 1])
            if _sc1.button(f"{_tl[_styp]} {_sname}", key=f"btn_nav_{_spos}", use_container_width=True, type="primary" if _is_active else "secondary"):
                page_pa = _page_key
            if _spos > 0:
    ...
    page_bot = st.radio("", _nav_bot, index=_nav_bot.index(_last) if _last in _nav_bot else None, label_visibility="collapsed")

    # Ustalanie aktywnej strony
    if page_pa is not None:
        page = page_pa
        st.session_state['last_page'] = page
        st.session_state['scroll_target'] = ""
        st.rerun()
    ...
    if page == "  ↳ Przerywnik hotel":
        ...
        st.text_input("Podtytuł (przy overline):", key="k_overline",
        ...
        st.text_area(
            "Fakty / Opis (Format: 'Etykieta: Wartość' lub wolny tekst):", height=160, key="k_facts",
            value=st.session_state.get('k_facts', 'Stolica: \nWaluta: \nRóżnica czasu: \nTemperatury: 16-20 stopni'),
        )
        ...
        for di in range(st.session_state.get('num_dist_pairs', 0)):
            for dk, dv in [(f'dist_a_{di}', ''), (f'dist_b_{di}', ''), (f'dist_km_{di}', '—'), (f'dist_time_{di}', '—')]:
                if dk not in st.session_state: st.session_state[dk] = dv
            with st.expander(f"Para {di+1}: {st.session_state.get(f'dist_a_{di}','')} → {st.session_state.get(f'dist_b_{di}','')}", expanded=True):
                ca, cb = st.columns(2)
...
for bk, bv in st.session_state.items():
    ...
    if bk.startswith(('up_', 'btn_', '$$', 'FormSubmitter', 'sb_', 'pa_add_')): 
        continue
    # Dodatkowa bariera: nie zapisujemy żadnych uploadowanych plików do tarczy
