...
NAPRAWIONO: wcięcia sekcji elif, _project_data w internal_keys, auto-save
"""
...
def set_focus(target_id):
    ...
    st.session_state['_user_edited'] = True
...
def section_template_manager(section_keys, file_prefix, default_filename, uploader_key, index=None):
    ...
    with st.expander("⚙️ Zarządzanie szablonem sekcji", expanded=False):
        ...
        with col2:
            st.download_button("↓ ZAPISZ", json_str, full_filename,
                               key=f"dl_{uploader_key}", use_container_width=True)
        with col3:
            uploaded_file = st.file_uploader("↑ WCZYTAJ", type=['json'], key=f"up_{uploader_key}",
...
def save_image_to_session_and_storage(key: str, raw_bytes: bytes, is_logo: bool = False) -> None:
...
with st.sidebar:
    ...
    st.radio(
        "Nawigacja górna", _nav_top, index=_top_index, key="nav_top_radio",
        label_visibility="collapsed",
        on_change=lambda: st.session_state.update({'last_page': st.session_state['nav_top_radio']})
    )

    st.markdown(
        f"<div style='display:flex;align-items:center;gap:6px;padding:3px 0 3px 4px;'>"
    ...
    if _n_attr > 0:
        for _ap in range(_n_attr):
            ...
            _ca.button(f"★ {_ap_name}", key=f"attrnav_{_ap}", use_container_width=True,
    ...
    st.radio(
        "Nawigacja dolna", _nav_bot, index=_bot_index, key="nav_bot_radio",
        label_visibility="collapsed",
...
with col_save:
    if "manual_save_btn" in st.session_state:
        del st.session_state["manual_save_btn"]
...
with col_form:
    ...
    # -----------------------------------------------------------------------
        ...
        st.text_area("Fakty (Format: 'Etykieta: Wartość'):", height=160, key="k_facts",
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["map_hide", "map_overline", "map_title", "map_subtitle", "map_desc",
                "map_zoom", "num_map_points", "map_dist_title", "ors_api_key", "num_dist_pairs"])
        map_keys = [
        ...
        if _ors_from_secrets:
            st.markdown("<div style='font-size:11px;color:#16a34a;margin-bottom:8px;padding:6px 10px;background:#f0fdf4;border-radius:4px;border-left:3px solid #16a34a;'>✓ Klucz API wczytany z konfiguracji aplikacji.</div>", unsafe_allow_html=True)
        else:
        ...
        for di in range(st.session_state.get('num_dist_pairs', 0)):
            for dk, dv in [(f'dist_a_{di}', ''), (f'dist_b_{di}', ''),
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["l_hide", "l_przesiadka", "l_port", "l_czas", "l_overline",
                "l_main", "l_sub", "m_route", "m_luggage", "f1", "f2", "f3", "f4", "l_desc", "l_extra"])
        l_keys = [
    ...
    # -----------------------------------------------------------------------
        ...
        if len(hotel_order) > 1:
            ...
            for pos, hi in enumerate(hotel_order):
                ...
                col_lbl.markdown(
                    f"<div style='padding:6px 10px;background:#f1f5f9;border-radius:4px;border-left:3px solid #003366;font-size:12px;color:#1e293b;'>"
                    f"<strong style='color:#003366;font-size:10px;text-transform:uppercase;letter-spacing:1px;'>Hotel {pos+1}</strong><br>{name}</div>",
                    unsafe_allow_html=True,
                )
                if pos > 0:
    ...
    # -----------------------------------------------------------------------
        ...
        if _i is None or _i >= _attr_count():
            ...
            if assigned_day != "Brak przypisania":
                if st.button(f"⬅️ Wróć do Programu ({assigned_day})", key=f"back_to_program_{_i}",
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["app_hide", "app_overline", "app_title", "app_subtitle", "app_features"])
        app_keys = ['app_hide', 'app_overline', 'app_title', 'app_subtitle',
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["brand_hide", "brand_overline", "brand_title", "brand_subtitle", "brand_features"])
        bra_keys = ['brand_hide', 'brand_overline', 'brand_title', 'brand_subtitle',
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["va_hide", "va_overline", "va_title", "va_subtitle", "va_text"])
        va_keys = ['va_hide', 'va_overline', 'va_title', 'va_subtitle',
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["pg_hide", "pg_overline", "pg_title", "pg_subtitle", "pg_text", "pg_features"])
        gif_keys = ['pg_hide', 'pg_overline', 'pg_title', 'pg_subtitle',
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["testim_hide", "testim_overline", "testim_title", "testim_subtitle", "testim_count"])
        opi_keys = ['testim_hide', 'testim_overline', 'testim_title', 'testim_subtitle',
        ...
        for i in range(st.session_state['testim_count']):
            for dk in [f"testim_head_{i}", f"testim_quote_{i}", f"testim_author_{i}", f"testim_role_{i}"]:
                if dk not in st.session_state:
                    st.session_state[dk] = ""
    ...
    # -----------------------------------------------------------------------
        ...
        _guard(["about_hide", "about_overline", "about_title", "about_sub", "about_desc", "team_count"])
        nas_keys = ['about_hide', 'about_overline', 'about_title', 'about_sub',
                    'about_desc', 'about_panel_title', 'about_panel_text', 'team_count', 'img_about_clients']
    ...
    # -----------------------------------------------------------------------
        ...
        st.info("📊 Twój projekt jest automatycznie zapisywany do bazy Supabase co 10 sekund.")
        st.markdown("---")
        ...
        upf = st.file_uploader("Wgraj projekt z dysku (.json)", type=['json'],
    ...
    if st.button("PRZYGOTUJ OFERTĘ DO POBRANIA", type="secondary", use_container_width=True):
        with st.spinner("Generowanie ostatecznego pliku oferty..."):
            ...
            client_html = (
                ...
                f'<style>body{{background:#f4f5f7;margin:0;}}'
                f'.presentation-wrapper{{height:100vh;overflow-y:auto;scroll-snap-type:y proximity;}}'
                ...
                f'@media print{{.client-export-btn{{display:none !important;}}'
                f'.presentation-wrapper{{height:auto !important;overflow:visible !important;}}}}'
                f'</style></head><body>'
    ...
    if st.session_state.get('show_link_info', False):
        st.info("Wyeksportuj plik HTML za pomocą przycisku wyżej i umieść go na serwerze swojej agencji.")
