...
with st.sidebar:
    ...
    # === SEKCJA PROJEKTY ===
    _acc_proj = st.session_state.get('color_accent', '#FF6600')
    st.markdown(
        f"<h3 style='color:{_acc_proj};font-size:16px;margin-bottom:15px;font-family:Montserrat,sans-serif;font-weight:700;'>ZARZĄDZANIE PROJEKTAMI</h3>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<div style='color:{_acc_proj};font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-top:15px;margin-bottom:5px;font-family:Montserrat,sans-serif;'>Wybierz projekt z bazy</div>",
        unsafe_allow_html=True,
    )
    if _all_offers:
        _proj_options = ["-- Wybierz --"] + [
