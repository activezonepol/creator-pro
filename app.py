...
from renderer import (
    ...
    format_flight_time, compute_layover_str,
)
...
def _check_login():
    ...
    _col_l, _col_m, _col_r = st.columns([3, 1, 3])
    with _col_m:
        st.markdown(
            """
...
# ---------------------------------------------------------------------------
# NAWIGACJA Z LINKÓW W PODGLĄDZIE (program → atrakcja)
...
if not st.session_state['project_selected']:
    st.markdown(
        ...
        /* Powiększenie tekstu na rozwiniętej liście (nazwy projektów) -
           ...
           komponentu, stąd szerokie selektory pokrywające różne warianty
           wewnętrznej struktury (stSelectboxVirtualDropdown / BaseWeb popover). */
...
if st.session_state['client_mode']:
    ...
    components.html(
        ...
        <div class="presentation-wrapper" style="height:100vh; overflow-y:auto;">{_slides_html}</div>{get_slide_nav_html(_acc_print)}{get_video_player_html()}""",
...
with st.sidebar:
    ...
    if _editing_id:
        try:
            ...
            _editing_code = ''
    st.markdown(
        f"<div style='background:#ffffff;border:2px solid {_acc_top};padding:12px 14px;"
        f"margin-bottom:15px;border-radius:4px;'>"
        f"<div style='font-size:14px;font-weight:700;color:{_acc_top};text-transform:uppercase;"
        f"letter-spacing:1px;margin-bottom:6px;'>Aktualnie edytujesz:</div>"
        f"<div style='font-size:25px;font-weight:700;color:{_acc_top};line-height:1.25;'>{_editing_name}</div>"
        f"<div style='font-size:14px;font-weight:700;color:{_acc_top};margin-top:4px;font-family:monospace;'>{_editing_code}</div>"
        f"</div>",
        unsafe_allow_html=True
    )
    ...
    with st.expander("ZARZĄDZANIE PREZENTACJĄ", expanded=False):
        ...
        if st.button("GENERUJ LINK DLA KLIENTA", type="primary", use_container_width=True, key="btn_send_offer_online"):
        ...
        with st.expander("LINKI WYSŁANYCH OFERT", expanded=False):
            try:
                ...
                _oferty_result = supabase.table('oferty_online').select(
                    'id, nazwa_klienta, nazwa_oferty, data_utworzenia, data_wygasniecia, data_aktualizacji, created_by, wersje'
...
with col_form:
    ...
    # -----------------------------------------------------------------------
        ...
        _country_gal = list_country_gallery(supabase, st.session_state.get('country_code', '') or 'XXX')
        _render_img_slot(st.container(), "Zdjęcie główne (duże, góra):", "img_hero_k", _country_gal)
        _c1, _c2 = st.columns(2)
        _render_img_slot(_c1, "Zdjęcie 2 (lewy dół):", "img_k_th1", _country_gal)
        _render_img_slot(_c2, "Zdjęcie 3 (prawy dół):", "img_k_th2", _country_gal)
    ...
    # -----------------------------------------------------------------------
        ...
        with st.expander("Wybierz punkty na trasie transferów", expanded=False):
            safe_number_input("Liczba par miejscowości:", key="num_dist_pairs", default=0, min_value=0, max_value=10, step=1)
    ...
    # -----------------------------------------------------------------------
        ...
        def _render_flight_leg(n, label, default_date=None):
            ...
            c1, c2 = st.columns(2)
            with c1:
                # Podpowiedź daty wylotu z terminu oferty - USTAWIANA TYLKO RAZ,
                # gdy pole jest jeszcze puste. Potem obowiązuje wartość operatora
                # i jest identyczna wszędzie (panel, podgląd, pełny ekran, PDF).
    ...
    # -----------------------------------------------------------------------
        ...
        for di in range(st.session_state.get('num_jaj_dist_pairs', 0)):
            ...
            with st.expander(f"Para {di+1}: {st.session_state.get(f'jaj_dist_a_{di}','')} → {st.session_state.get(f'jaj_dist_b_{di}','')}",
                ...
                ca, cb = st.columns(2)
                with ca:
                    safe_text_input("Miejsce A:", key=f"jaj_dist_a_{di}")
                with cb:
                    safe_text_input("Miejsce B:", key=f"jaj_dist_b_{di}")
    ...
    # -----------------------------------------------------------------------
        ...
        _country_gal = list_country_gallery(supabase, st.session_state.get('country_code', '') or 'XXX')
        _render_img_slot(st.container(), "Zdjęcie tła (16:9):", "sek_2_img", _country_gal)
    ...
    # -----------------------------------------------------------------------
        ...
        safe_text_area("Cytat (bez cudzysłowów):", height=80, key="esg_quote",
                       help="Cytat z artykułu branżowego o ESG (np. Think MICE).")
        safe_text_input("Źródło cytatu (np. 'Think MICE'):", key="esg_quote_source")
    ...
    # -----------------------------------------------------------------------
        ...
        with st.expander("POLA METRYK I CERTYFIKATÓW (rozwiń, aby edytować)", expanded=False):
            for _mi in range(1, 5):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    safe_text_input(f"Treść główna (Pole {_mi}):", key=f"about_m{_mi}_number",
                                    help="Duża górna treść — liczba lub tekst (np. '724', '1997', 'SOIT', 'Pakiet compliance').")
                with c2:
                    safe_text_input(f"Dopełnienie (Pole {_mi}):", key=f"about_m{_mi}_value",
                                    help="Mniejsze dopełnienie obok treści głównej (np. 'PLN', 'os.', '%').")
                safe_text_input(f"Etykieta mała (Pole {_mi}):", key=f"about_m{_mi}_label",
