...
with st.sidebar:
    ...
    # -----------------------------------------------------------------------
        ...
        map_zoom = st.slider("Przybliżenie mapy (Zoom):", 4, 10, key="map_zoom")
    ...
    # -----------------------------------------------------------------------
        ...
        for (f_key, c_key, s_key, label) in [
            ...
            c3.number_input("Rozmiar (px)", key=s_key)
    ...
    # -----------------------------------------------------------------------
        ...
        for k, v in st.session_state.items():
            if k in EXCLUDE_EXPORT_KEYS:
                continue
            if k.startswith(('FormSubmitter', '$$', 'up_', 'fn_', 'dl_', 'btn_')):
                continue
            if k in ('tyt_hero', 'tyt_logo_az', 'tyt_logo_cli',
            ...
            if re.match(
                ...
                r'atr_hero|atr_th1|atr_th2|atr_th3|opi_img|nas_img)_\d+$', k
            ):
                continue
            try:
        ...
        st.download_button(
            "POBIERZ PLIK PROJEKTU (JSON)", json.dumps(proj),
