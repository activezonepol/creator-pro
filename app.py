...
def _check_or_acquire_lock():
    ...
    try:
        ...
        my_session = st.session_state['session_id']
        now = datetime.now()
...
if st.session_state['lock_status'] == 'readonly':
    ...
    st.markdown(
        ...
        <div style='background:#fef3c7;border-left:5px solid #f59e0b;
                    padding:25px;margin:20px 0;border-radius:8px;'>
            <h2 style='color:#92400e;margin-top:0;'>
                ⚠️ Projekt już otwarty w innej karcie
            </h2>
            <p style='color:#78350f;font-size:15px;line-height:1.6;'>
                Wykryto, że ten projekt jest aktualnie edytowany w innej karcie
