...
def generate_map_data(points, zoom=6, _depth=0):
    ...
    if crop_h < pts_max_y - pts_min_y + pad * 2:
        crop_h = pts_max_y - pts_min_y + pad * 2
        crop_w = crop_h / 0.85
...
def get_local_css(return_str=False):
    ...
    try: fs_h1, fs_h2, fs_sub, fs_txt, fs_met = s.get('font_size_h1'), s.get('font_size_h2'), s.get('font_size_sub'), s.get('font_size_text'), int(s.get('font_size_metric', 16))
    except: fs_h1, fs_h2, fs_sub, fs_txt, fs_met = 48, 36, 26, 14, 16
    ...
    css = f"""<style>{fonts_css}@import url('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css');{client_css}
        ...
        .floating-btn {{ position: absolute; bottom: 25px; left: 25px; background: {c_acc}; color: white !important; padding: 12px 24px; border-radius: 4px; text-decoration: none !important; font-family: '{f_h2}'; font-weight: 700; font-size: 10px; text-transform: uppercase; box-shadow: 0 4px 15px rgba(0,0,0,0.3); z-index: 10; transition: opacity 0.2s; opacity: 0.9; }}
