...
defaults = {
    ...
    'l_hide': False, 'l_przesiadka': False, 'l_port': '',
    'l_przesiadka_pow': False, 'l_port_pow': '',
    'l_czas': '', 'l_czas_pow': '', 'l_czas_manual': False, 'l_czas_pow_manual': False,
    'f1_data_manual': False, 'f2_data_manual': False, 'f3_data_manual': False, 'f4_data_manual': False,
    'l_overline': 'PRZELOT', 'l_main': 'JAK LECIMY?',
    ...
    'map_points_title': 'PUNKTY NA TRASIE',
    'num_map_points': 3,
    ...
    'img_brand_1': 'https://ckmmtjuolhhboruujdni.supabase.co/storage/v1/object/public/nexa-images/default_user/BRANDING/brand1.png',
    'img_brand_2': 'https://ckmmtjuolhhboruujdni.supabase.co/storage/v1/object/public/nexa-images/default_user/BRANDING/brand2.jpg',
    'img_brand_3': 'https://ckmmtjuolhhboruujdni.supabase.co/storage/v1/object/public/nexa-images/default_user/BRANDING/brand3.jpg',
...
_ADMIN_DATA_KEYS = {
    ...
    'auto_map_points', 'map_zoom_adjust',
...
_DYNAMIC_DATA_PATTERNS = [
    ...
    r'^hvideo_url_\d+$', r'^hvideo_text_\d+$', r'^hvideo_start_\d+$', r'^hvideo_end_\d+$',
    r'^h_day_\d+$',
...
Napisz sam opis, bez żadnego wstępu ani komentarza."""
    ...
    # Główny model: gemini-3.5-flash (4 próby z rosnącym odstępem).
    # Jeśli wszystkie próby zawiodą (typowo z powodu przeciążenia 503),
    # jako OSTATECZNOŚĆ próbujemy gemini-3.1-pro - inny poziom modelu,
    # zwykle mniej oblegany niż najnowszy Flash, prawdopodobnie inna pula
    # zasobów. Próbowany TYLKO RAZ (Pro jest znacznie droższy niż Flash,
    # nie chcemy tego nadużywać przy każdym błędzie).
    import time as _time

    def _call_gemini(model_name: str, retries: int, base_delay: int):
        _url = (
        ...
        for _attempt in range(retries):
            try:
                ...
                _err = f"Błąd API Gemini ({e.code}, model {model_name}): {_detail}"
                if e.code in (503, 429) and _attempt < retries - 1:
                    _time.sleep(base_delay * (_attempt + 1))
                    continue
                return None, _err
            except Exception as e:
                _err = f"Błąd połączenia z Gemini ({model_name}): {str(e)[:150]}"
...
def build_presentation(current_page="Strona Tytułowa", export_mode=False, active_attr_idx=None, active_hotel_idx=None):
    """
    ...
    if _should_render('slide-loty', current_page, export_mode):
        ...
        hp.append(_shtml(f"""{lh}
        <div class="premium-layout">
            <div class="photo-col" style="flex:40;">{iml}</div>
            <div class="info-col" style="flex:60; padding-top:6px; justify-content:flex-start;">
                ...
                <div class="title-sub" style="margin-bottom:10px;">{str(get_data('l_sub','')).replace(chr(10),'<br>')}</div>
                {h_d}
                
                <div class="metric-grid" style="grid-template-columns: 2.5fr 1fr; gap: 24px; margin-top:4px; padding-top:6px;">
    ...
    for i in _hotel_order:
        if _should_render(f'slide-hotel-{i}', current_page, export_mode, active_hotel_idx=active_hotel_idx) and not get_data(f'h_hide_{i}', False):
            ...
            if _hmd:
                if export_mode:
                    _h_prog_id = f"program_day_{int(_hmd.group(1)) - 1}"
                    bb_h = (
            ...
            hp.append(_shtml(f"""{lh}<div class="premium-layout" id="slide-hotel-{i}" style="align-items:stretch;">
                <div style="flex:40; display:flex; flex-direction:column; gap:12px;">
                    ...
                    <div style="flex:2; position:relative; border-radius:8px; overflow:hidden; border:1px solid #eee; background-color:#fcfcfc;">{h1b_html}{bb_h}{_hvid_btn}</div>
                </div>
        ...
        for st_idx in range(0, nd, 3):
            ...
            for i in range(3):
                ...
                if di < nd:
                        ...
                        if d_match and int(d_match.group(1)) == di + 1:
                            ...
                            if get_data(f"ahide_{ai}"):
                                ...
                                if export_mode:
                                    ...
                                    _link_attrs = (
                                        f"href='javascript:void(0)' "
                                        f"onclick='var _a=window.parent.document.createElement(\"a\");"
                                        f"_a.href=\"?nexa_goto=attr&nexa_idx={ai}\";"
                                        f"window.parent.document.body.appendChild(_a);"
                                        f"_a.click();_a.remove();'"
                                    )
                    ...
                    _hotel_order_prg = get_data('hotel_order', [])
                    if not _hotel_order_prg:
                        _hotel_order_prg = list(range(get_data('num_hotels', 0)))
                    for hi in _hotel_order_prg:
                        _h_day = str(get_data(f"h_day_{hi}") or "")
...
def get_video_player_html():
    ...
    #nexa-vid-slot { width:100%; height:100%; }
    ...
    @media print { .nexa-vid-ov { display:none !important; } }
    </style>
    <div class="nexa-vid-ov" id="nexa-vid-ov" onclick="if(event.target===this)nexaCloseVideo()">
      <div class="nexa-vid-box">
        <button class="nexa-vid-x" onclick="nexaCloseVideo()" aria-label="Zamknij film">&times;</button>
        <div id="nexa-vid-slot"></div>
      </div>
    </div>
    <script>
    (function(){
      function secs(t){ if(!t) return 0; t=(''+t).trim();
        if(t.indexOf(':')>=0){ var p=t.split(':'); if(p.length===3) return (+p[0])*3600+(+p[1])*60+(+p[2]); return (+p[0])*60+(+p[1]); }
        return parseInt(t,10)||0; }
      function ytId(u){ var m=null;
        if(u.indexOf('youtu.be/')>=0) m=u.split('youtu.be/')[1];
        else if(u.indexOf('watch?v=')>=0) m=u.split('watch?v=')[1];
        else if(u.indexOf('/embed/')>=0) m=u.split('/embed/')[1];
        else if(u.indexOf('/shorts/')>=0) m=u.split('/shorts/')[1];
        if(!m) return null; m=m.split('&')[0].split('?')[0].split('/')[0];
        return m.length>=10?m:null; }
      function vmId(u){ if(u.indexOf('vimeo.com/')<0) return null;
        var a=u.split('vimeo.com/')[1].split('?')[0].split('/'); var id=a[a.length-1]||a[0];
        return (id && (''+parseInt(id,10))===id)?id:null; }
      window.nexaPlayVideo=function(url,start,end){
        var s=secs(start), e=secs(end), src=null, y=ytId(url), v=vmId(url);
        if(y){ src='https://www.youtube.com/embed/'+y+'?autoplay=1&rel=0'; if(s)src+='&start='+s; if(e)src+='&end='+e; }
        else if(v){ src='https://player.vimeo.com/video/'+v+'?autoplay=1'; if(s)src+='#t='+s+'s'; }
        if(src){ document.getElementById('nexa-vid-slot').innerHTML='<iframe src="'+src+'" allow="autoplay; fullscreen" allowfullscreen></iframe>';
          document.getElementById('nexa-vid-ov').classList.add('open'); }
        else { window.open(url,'_blank'); } };
      window.nexaCloseVideo=function(){ document.getElementById('nexa-vid-ov').classList.remove('open');
        document.getElementById('nexa-vid-slot').innerHTML=''; };
