...
def generate_map_data(points, zoom=6, _depth=0):
    ...
    if not geo_pts:
        return None, [{'name': p['name'], 'x': p['x'], 'y': p['y'], 'conn': p.get('conn', 'Brak')} for p in points]
    ...
    for p in points:
        if p.get('symbolic'):
            final_points.append({'name': p['name'], 'x': p['x'], 'y': p['y'], 'conn': p.get('conn', 'Brak')})
        else:
            px, py = pixel_points[geo_idx]['px'], pixel_points[geo_idx]['py']
            geo_idx += 1
            x_pct, y_pct = (px - left) / max(1, final_w) * 100, (py - top) / max(1, final_h) * 100
            final_points.append({'name': p['name'], 'x': x_pct, 'y': y_pct, 'conn': p.get('conn', 'Brak')})
...
def get_local_css(return_str=False):
    ...
    try: fs_h1_val, fs_h2_val, fs_sub_val, fs_txt_val, fs_met = int(s.get('font_size_h1')), int(s.get('font_size_h2')), int(s.get('font_size_sub')), int(s.get('font_size_text')), int(s.get('font_size_metric', 16))
...
def build_presentation(current_page="Strona Tytułowa", export_mode=False):
    ...
    try: fs_h2_val = int(float(s.get('font_size_h2', 36)))
    except: fs_h2_val = 36
    ...
    if not s.get('map_hide', False):
        ...
        for i, pt in enumerate(auto_pts):
            ...
            if i < len(auto_pts) - 1 and conn != 'Brak':
                next_pt = auto_pts[i+1]
                x2 = next_pt.get('x', 0)
                y2 = next_pt.get('y', 0)
                is_flight = conn and "Przelot" in str(conn)
                ...
                if is_flight:
                    dx, dy = x2 - x1, y2 - y1
                    rot = math.degrees(math.atan2(dy, dx)) + 45
